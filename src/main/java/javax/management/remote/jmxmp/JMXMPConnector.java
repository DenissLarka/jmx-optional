...
public class JMXMPConnector extends GenericConnector
	 ...
	 * @throws MalformedURLException    if <code>address</code> is not
	 *                                  a valid URL for the JMXMP connector.
	 * @throws IOException              if the connector cannot work for another reason.
	 */
	...
	private void validateAddress() throws IOException {
		...
		if (!protocolName.equalsIgnoreCase(address.getProtocol())) {
			throw new MalformedURLException("Unknown protocol: " + address.getProtocol());
		}
