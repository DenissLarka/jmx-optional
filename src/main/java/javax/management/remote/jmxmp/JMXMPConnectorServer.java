...
public class JMXMPConnectorServer extends GenericConnectorServer {
	 ...
	 *
	 * @throws IOException if the connector server cannot be created
	 *                     for some reason.
	 */
	 ...
	 * @throws IllegalArgumentException if <var>env</var> contains
	 *                                  some invalid values.
	 * @throws IOException              if the connector server cannot be created
	 *                                  for some reason.
	 */
	...
	private static Map completeEnv(JMXServiceURL address, Map env) throws IOException {
		...
		newEnv.put(MESSAGE_CONNECTION_SERVER, new SocketConnectionServer(address, newEnv));
		return newEnv;
	}
}
