	...
	public void start(Map env) throws IOException {
		...
		try {
			...
			if (wildcard) {
				...
				o = ct.newInstance(new Object[] {Integer.valueOf(port)});
			} else {
				Constructor ct = c.getDeclaredConstructor(new Class[] {String.class, int.class});
				o = ct.newInstance(new Object[] {host, Integer.valueOf(port)});
			}
		}
		catch (Exception ee) {
			// OK.
		...
		if (o != null && DefaultConfig.getServerReuseAddress(newEnv)) {
			try {
				...
				// bind
				m2.invoke(ss, new Object[] {o, Integer.valueOf(DEFAULT_BACKLOG)});
			}
