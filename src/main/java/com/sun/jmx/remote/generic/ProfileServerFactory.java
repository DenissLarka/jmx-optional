 ...
 * <p>Each profile is created by an instance of {@link ProfileServerProvider}.
 * This instance is found as follows. Suppose the given
 * <code><em>profile</em></code> looks like <code>TLS</code>. Then the factory
 * will attempt to find the appropriate {@link ProfileServerProvider} for
 ...
 * the appropriate {@link ProfileServerProvider} for <code><em>sasl</em></code>.
 ...
 * class loader that loaded the <code>ProfileServerFactory</code> class
 * is used.</p>
 ...
 * ProfileServerProvider#createProfile(String, Map) createProfile}
 * on the provider.</p>
 * <p>The <code>Map</code> parameter passed to the
 * <code>ProfileServerProvider</code> is a new read-only copy of the
 * <code>environment</code> parameter to {@link
 * #createProfile(String, Map)
 * ProfileServerFactory.createProfile}, or an empty <code>Map</code> if
 ...
 * loaded the <code>ProfileServerFactory</code> class.</p>
 */
public class ProfileServerFactory {
	...
	private static final String resolvePkgs(Map env) {
		...
		if (pkgs == null) {
			pkgs = System.getProperty(PROFILE_PROVIDER_PACKAGES);
			...
			if (!(obj instanceof ProfileServerProvider)) {
				final String msg =
						"Provider not an instance of " +
								ProfileServerProvider.class.getName() + ": " +
