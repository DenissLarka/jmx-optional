...
public class TLSServerHandler implements ProfileServer {
	...
	public void initialize(MessageConnection mc, Subject s) throws IOException {
		...
		if (bundledJSSE) {
			...
			if (logger.traceOn()) {
				logger.trace("initialize", "TLS: Socket Want Client Authentication = " + getWantClientAuth(ts));
			}
		}
	}
	...
	public Subject activate() throws IOException {
		...
		if (session != null) {
			if (logger.traceOn()) {
				...
				if (bundledJSSE) {
					logger.trace("activate", "TLS: getProtocol = " + getProtocol(session));
				}
			}
			...
			try {
				...
				if (certificate != null && certificate[0] != null && certificate[0] instanceof X509Certificate cert) {
					Principal p = cert.getSubjectX500Principal();
