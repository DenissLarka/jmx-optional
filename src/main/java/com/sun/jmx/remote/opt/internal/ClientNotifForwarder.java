...
public abstract class ClientNotifForwarder {
	...
	removeNotificationListener(ObjectName name, NotificationListener listener) throws ListenerNotFoundException, IOException {
	...
	private class NotifFetcher implements Runnable {
		public void run() {
			...
			while (!shouldStop()) {
				...
				if (nr == null) {
					break; // nr == null means got exception
				}
				...
				synchronized (ClientNotifForwarder.this) {
					...
					for (int i = 0; i < len; i++) {
						...
						if (!listenerID.equals(mbeanRemovedNotifID)) {
							final ListenerInfo li = (ListenerInfo) infoList.get(listenerID);
							if (li != null) {
		...
		void dispatchNotification(TargetedNotification tn, Integer myListenerID, Map listeners) {
			...
			final ListenerInfo li = (ClientListenerInfo) listeners.get(listenerID);
		...
		*/
		private NotificationResult fetchOneNotif() {
			...
			if (notFoundCount > 0) {
				final String msg =
						"Dropped " + notFoundCount + " notification" + (notFoundCount == 1 ? "" : "s") + " because classes were missing locally";
